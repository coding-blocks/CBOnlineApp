...
class SectionDetailsAdapter(
    ...
    inner class CourseViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindView(data: CourseSection, starter: DownloadStarter) {
            ...
            sectionWithContentDao.getContentWithSectionId(data.id).getDistinct()
                .observe(activity, Observer<List<CourseContent>> {
                        ...
                        if (!data.premium || premium && ((courseStartDate.toLong() * 1000) < System.currentTimeMillis())) {
                            if (sectionComplete == it.size) {
                                itemView.lectures.text =
                                    "$sectionComplete/${it.size} Lectures Completed"
        ...
        doAsync {
            ...
            Clients.onlineV2JsonApi.setProgress(p).enqueue(retrofitCallback { throwable, response ->
                ...
                response?.body().let {
                    ...
                    when (contentable) {
                        "lecture" -> thread {
                            contentDao.updateProgressLecture(
                        ...
                        "document" ->
                            thread {
                                contentDao.updateProgressDocuemnt(
                                    sectionId, contentId, "DONE", progressId
                                        ?: ""
                                )
                            }
                        "video" ->
                            thread {
                                contentDao.updateProgressVideo(
                                    sectionId, contentId, "DONE", progressId
                                        ?: ""
                                )
                            }
