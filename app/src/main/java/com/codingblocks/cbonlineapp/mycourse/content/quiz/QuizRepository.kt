...
class QuizRepository(private var contentDao: ContentDao) {
    ...
    suspend fun fetchQuizAttempt(quizAttemptId: String) =
        safeApiCall { CBOnlineLib.onlineV2JsonApi.getQuizAttemptById(quizAttemptId) }

    suspend fun submitQuiz(quizAttemptId: String) =
        safeApiCall { CBOnlineLib.onlineV2JsonApi.submitQuizById(quizAttemptId) }
}
